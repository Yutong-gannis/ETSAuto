...
class Supercombo_onnx(object):
    def __init__(self, onnx_path):
        ...
        self.accuracy = 'float16'

        self.pulse_desire_data = np.zeros((1, 100, 8)).astype(self.accuracy)  # 意图脉冲数据
        self.desire_data = np.zeros((1, 8)).astype(self.accuracy)  # 此时意图
        '''
        0: 直行 1: 左转 2: 右转 3: 左变道 4: 右变道 5: 保持左转 6: 保持右转 7: Null
        '''
        self.traffic_convention_data = np.array([[0, 1]]).astype(self.accuracy)
        self.nav_features_data = np.zeros((1, 256)).astype(self.accuracy)
        self.initial_state_data = np.zeros((1, 512)).astype(self.accuracy)
        self.history_data = np.zeros((1, 99, 128)).astype(self.accuracy)
    ...
    def infer(self, img, desire, nav_feature=None):
        ...
        if len(self.parsed_images) >= 2:
            ...
            data_big = parsed_arr_big.astype(self.accuracy)
