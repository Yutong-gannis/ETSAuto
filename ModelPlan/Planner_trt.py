...
class Supercombo_trt_1(object):
    def __init__(self, engine_path):
        ...
        self.pulse_desire_data = np.zeros((1, 8)).astype('float16')
        ...
        self.history_data = np.zeros((1, 512)).astype('float16')
    ...
    def infer(self, img, desire):
        ...
        if len(self.parsed_images) >= 2:
            ...
            self.desire_data = np.zeros((1, 8)).astype('float16')
            self.desire_data[desire] = 1
            self.inputs[0]['host'] = np.ravel(data)
            self.inputs[1]['host'] = np.ravel(self.desire_data)
            self.inputs[2]['host'] = np.ravel(self.traffic_convention_data)
            self.inputs[3]['host'] = np.ravel(self.history_data)
