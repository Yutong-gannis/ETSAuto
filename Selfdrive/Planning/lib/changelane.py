...
class ChangeLane_Helper:
    ...
    def plan_change_subject(self, trajectory, line_l, lane_width, desire, condition_dict):
        ...
        trajectory_total = np.vstack((self.trajectory_change[:5, :], line_target[math.ceil(self.trajectory_change[-1, 0]) * 2 + 1:, :]))
    ...
    def publish(self):
        ...
        save_pkl(os.path.join(project_path, 'Message/temp/states.pkl'), states_dict)
